...
app.post("/register", async (req, res) => {
  ...
  if (!username || !email || !password || !dateNaissance) {
    return res
      .status(400)
      .send(
        "Nom d'utilisateur, adresse email, mot de passe requis et date de naissance requis"
      );
  }
  ...
  try {
    ...
    const query =
      "INSERT INTO users (username, email, password, dateNaissance) VALUES (?, ?, ?, ?)";
    db.query(
      query,
      [username, email, hashedPassword, dateNaissance],
      (err, result) => {
        if (err) {
...
app.get("/checkUsername", (req, res) => {
  ...
  if (!username) {
    return res
      .status(400)
      .json({ error: "Le nom d'utilisateur manquant dans la requête." });
  }
...
app.get("/checkEmail", (req, res) => {
  ...
  db.query(query, [email], (err, result) => {
    if (err) {
      console.error(
        "Erreur lors de la vérification de l'adresse email : " + err.message
      );
      return res
        .status(500)
        .json({ error: "Erreur lors de la vérification de l'adresse email." });
    }
...
app.get("/profil", isLoggedIn, (req, res) => {
  if (req.session.user) {
    ...
    const { id, username, email, dateNaissance, genre, pays, image } =
      req.session.user;
    res.json({ id, username, email, dateNaissance, genre, pays, image });
  }
});
...
app.post("/profil", isLoggedIn, upload.single("image"), async (req, res) => {
  if (req.session.user) {
    ...
    const { username, email, password, dateNaissance, genre, pays } = req.body;
    ...
    if (dateNaissance) {
      ...
      const updateQuery = `UPDATE users SET username=?, email=?${updatePasswordPart}, dateNaissance=?, genre=?, pays=?, image=? WHERE id=?`;
            ...
            if (password) {
              ...
              db.query(
                ...
                (hashErr, hashResult) => {
                  if (hashErr) {
                    console.error(
                      "Erreur lors du hachage du nouveau mot de passe :",
                      hashErr.message
                    );
                    res.status(500).json({
                    ...
                    res
                      .status(200)
                      .json({ message: "Mise à jour du profil réussie" });
                  }
                }
              );
            } else {
      ...
      const updateQuery =
        "UPDATE users SET username=?, email=?, genre=?, pays=?, image=? WHERE id=?";
...
app.get("/plateformes", (req, res) => {
  db.query("SELECT * FROM plateformes", (err, results) => {
    if (err) {
      return res
        .status(500)
        .json({ error: "Erreur lors de la récupération des plateformes." });
    }
    res.json(results);
  });
});
...
app.get("/developpeurs", (req, res) => {
  db.query("SELECT * FROM developpeurs", (err, results) => {
    if (err) {
      return res
        .status(500)
        .json({ error: "Erreur lors de la récupération des développeurs." });
    }
    res.json(results);
  });
});

app.get("/editeurs", (req, res) => {
  db.query("SELECT * FROM editeurs", (err, results) => {
    if (err) {
      return res
        .status(500)
        .json({ error: "Erreur lors de la récupération des éditeurs." });
    }
    res.json(results);
  });
});
...
app.get("/jeux/:id/developpeurs", (req, res) => {
  const jeuId = req.params.id;

  // Requête SQL pour récupérer les développeurs associés à un jeu spécifique
  const sql = `
        SELECT d.nom
        FROM developpeurs d
        JOIN jeux_developpeurs jd ON d.id = jd.id_developpeur
        WHERE jd.id_jeu = ?
    `;
  // Exécution de la requête SQL avec le paramètre du jeuId
  db.query(sql, [jeuId], (err, results) => {
...
app.get("/jeux/:id/editeurs", (req, res) => {
  const jeuId = req.params.id;

  // Requête SQL pour récupérer les éditeurs associés à un jeu spécifique
  const sql = `
        SELECT e.nom
        FROM editeurs e
        JOIN jeux_editeurs je ON e.id = je.id_editeur
        WHERE je.id_jeu = ?
    `;
  // Exécution de la requête SQL avec le paramètre du jeuId
  db.query(sql, [jeuId], (err, results) => {
...
app.post("/notes", async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Évaluation ajoutée ou mise à jour avec succès" });
  } catch (error) {
    console.error(
      "Erreur lors de l'ajout ou de la mise à jour de l'évaluation :",
      error
    );
    res.status(500).json({ error: "Erreur côté serveur" });
  }
...
app.get("/notes/moyenne/:id_jeu", (req, res) => {
  ...
  try {
    ...
    db.query(query, [id_jeu], (error, results) => {
      if (error) {
        console.error(
          "Erreur lors de la requête SQL pour la moyenne des notes. Requête :",
          query
        );
        res.status(500).json({ error: "Erreur côté serveur" });
      } else {
        ...
        console.log(
          "Résultat de la requête SQL pour la moyenne des notes :",
          results
        );
        ...
        if (results && results.length > 0) {
          console.log(
            "Moyenne des notes pour le jeu avec l'ID",
            id_jeu,
            ":",
            results
          );
...
app.get("/commentaires/:id_jeu", async (req, res) => {
  ...
  try {
    ...
    db.query(query, [id_jeu], (error, results) => {
      if (error) {
        console.error(
          "Erreur lors de la requête SQL pour les commentaires. Requête :",
          query
        );
        res.status(500).json({ error: "Erreur côté serveur" });
      } else {
        // Utilise des logs pour vérifier les résultats
        console.log(
          "Résultat de la requête SQL pour les commentaires :",
          results
        );
